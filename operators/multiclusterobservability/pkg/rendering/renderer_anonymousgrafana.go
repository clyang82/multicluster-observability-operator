...
func (r *MCORenderer) newAnonymousGranfanaRenderer() {
	r.renderAnonymousGrafanaFns = map[string]rendererutil.RenderFn{
		"Deployment":            r.renderAnonymousGrafanaDeployments,
...
func (r *MCORenderer) renderAnonymousGrafanaDeployments(res *resource.Resource,
	...
	dep.Name = config.GetOperandName(config.AnonymousGrafana)
	dep.Spec.Replicas = config.GetReplicas(config.AnonymousGrafana, r.cr.Spec.AdvancedConfig)
...
func (r *MCORenderer) renderAnonymousGrafanaTemplates(templates []*resource.Resource,
	namespace string, labels map[string]string) ([]*unstructured.Unstructured, error) {
	uobjs := []*unstructured.Unstructured{}
	for _, template := range templates {
		render, ok := r.renderAnonymousGrafanaFns[template.GetKind()]
		if !ok {
